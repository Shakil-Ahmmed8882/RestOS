  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          ...
          >
            ...
            <BaseSkeleton className="mt-1.5 h-2.5 w-20" />
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        <BaseSkeleton className="h-9 flex-1 rounded-xl" />
        ...
        <div className="border-b border-zinc-100 bg-zinc-50 px-5 py-3 dark:border-white/[0.04] dark:bg-zinc-900/40">
          <div className="flex gap-6">
            {[160, 40, 80, 80, 70, 70, 80].map((w, i) => (
              <BaseSkeleton key={i} className="h-3" style={{ width: w }} />
            ))}
          </div>
        </div>
        {Array.from({ length: 6 }).map((_, i) => (
          ...
          >
            <div className="flex items-center gap-3" style={{ width: 160 }}>
              <BaseSkeleton className="h-10 w-10 shrink-0 rounded-xl" />
