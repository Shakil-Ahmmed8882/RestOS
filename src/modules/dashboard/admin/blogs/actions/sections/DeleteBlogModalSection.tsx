...
import { useOptimisticDeleteBlog } from "../hooks/useOptimisticDeleteBlog";
...
export function DeleteBlogModalSection(props: Props) {
  const { blogId, blogTitle } = props;
  const { close } = useBlogActionsSelector();
  const { run, isLoading: deleting } = useOptimisticDeleteBlog();

  const handleDelete = async () => {
    const ok = await run(blogId);
    if (ok) close();
  };
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            <span className="font-semibold text-foreground">{blogTitle}</span>{" "}
