      ...
      <div className="card flex-shrink-0  w-full flex-1 h-screen  justify-center ">
        ...
        <form onSubmit={handleSignin} className={`mx-8`}>
          ...
          <div className={`form-control mt-6`}>
            <button className={`btn ${theme == 'dark'?'bg-[white]':'bg-primaryColor hover:bg-[#cb3890] text-[white]'}`} type="submit">Sign in</button>
          </div>
        </form>
        <div className={`flex gap-3 items-center justify-center pt-7  ${theme == 'dark'?'text-[white]':''}`}>
                        Don&apos;t have account? <Link to='/sign-up' className={` ${theme == 'dark'?'text-[white] underline':'text-[blue]'}`}>Sign up</Link>
            </div>
        ...
        <button className={`flex justify-center items-center gap-2 ${theme == 'dark'?'text-[white]':''} py-9`}>
          ...
          Sign in with google
            ...
            <div className={`${theme == 'dark'?'bg-dark-sign-in text-[white]':'bg-light-food text-[black]'} w-full flex-1 h-screen flex flex-col p-3 justify-center items-center`}>
